...
export async function up(knex: Knex): Promise<void> {
  return knex.raw(`
    -- Update Active Shops Count Logic in Daily Statistics View
    -- This migration adds created_at filtering to the Active shops count calculation
    -- as requested to match the logic pattern used in other statistics
    ...
    SELECT 
        ...
        -- Active shops count (not suspended, filtered by created_at date)
        COALESCE(
            (SELECT COUNT(*) 
             FROM shops s 
    ...
    COMMENT ON COLUMN daily_statistics_view.shops_active IS 'Active shops (not suspended) created on the date - UPDATED with created_at filtering';
...
export async function down(knex: Knex): Promise<void> {
  // Revert back to the previous logic without created_at filtering for Active shops count
  return knex.raw(`
    CREATE OR REPLACE VIEW daily_statistics_view AS
    SELECT 
        DATE(stats_date) AS date,
        ...
        -- REVERTED: Back to original logic without created_at filtering
        COALESCE(
            (SELECT COUNT(*) 
             FROM shops s 
