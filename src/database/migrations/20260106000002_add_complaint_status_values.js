...
function up(knex) {
    return __awaiter(this, void 0, void 0, function () {
        var findEnumResult, enumName, newValues, _i, newValues_1, value, existsResult;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    findEnumResult = (_a.sent());
                    if (!(findEnumResult.rows && findEnumResult.rows.length > 0)) return [3 /*break*/, 6];
                    enumName = findEnumResult.rows[0].enum_name;
                    newValues = ['admin_review', 'rejected', 'cancelled'];
                    _i = 0, newValues_1 = newValues;
                    _a.label = 2;
                case 2:
                    if (!(_i < newValues_1.length)) return [3 /*break*/, 6];
                    value = newValues_1[_i];
                    return [4 /*yield*/, knex.raw("\n        SELECT 1 FROM pg_enum \n        WHERE enumlabel = ? \n        AND enumtypid = (SELECT oid FROM pg_type WHERE typname = ?);\n      ", [value, enumName])];
                case 3:
                    existsResult = (_a.sent());
                    if (!(!existsResult.rows || existsResult.rows.length === 0)) return [3 /*break*/, 5];
                    ...
                    return [4 /*yield*/, knex.raw("ALTER TYPE \"".concat(enumName, "\" ADD VALUE '").concat(value, "';"))];
                case 4:
                    // Use string interpolation for enum name (safe because we control it from query result)
                    // PostgreSQL doesn't support parameterized queries for ALTER TYPE
                    _a.sent();
